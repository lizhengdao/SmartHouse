...
public class MessagePool {
    ...
    public static void addMessage(MessageBean msg) {
        ...
        for (IMMessageBean immsgBean : imMessageBeans) {
            if (immsgBean.getUserId().equals(msg.getUserId())) {
                immsgBean.getMessages()
                        .add(immsgBean.getMessages().size(), msg);
                flag = false;
                break;
            }
        }
    ...
    public static void setReadStateTrue(int position) {
        if (position < 0 || imMessageBeans == null) {
            return;
        }
        
        int size = imMessageBeans.size();
        if (size == 0 || position >= size) {
            return;
        }
        
        IMMessageBean imMessageBean = imMessageBeans.get(position);
        ArrayList<MessageBean> messages = imMessageBean.getMessages();
