	...
	public static Bitmap getBitmapFormURL(String imageUrl,int type,int newWidth,int newHeight){
		...
		try {
			...
			if (type == SCALEIMG2) {
				...
				opt.inSampleSize = calculateInSampleSize(opt, newWidth, newHeight);
...
//			AbLogUtil.d(AbImageUtil.class,""+e.getMessage());
    ...
    */
	...
	public static Bitmap toRoundBitmap(Bitmap bitmap, int pixel) {
		...
		if (width <= height) {
			if(pixel >= width/2)roundPx = width / 2;
			else roundPx = pixel;
			top = 0;
			bottom = width;
			left = 0;
			...
			if(pixel >= height/2)roundPx = width / 2;
			else roundPx = pixel;
...
//						AbLogUtil.d(AbImageUtil.class,"Bitmap释放"+bitmap.toString());
     ...
     * @author shaowei.ma
     * @date 2014年9月19日
     * @param options 携带图片尺寸信息的BitmapFactory.Options
     * @param reqWidth 图片目标宽度
     * @param reqHeight 图片目标高度
     * @return
     */
