...
public class FillBankCardInfoActivity extends BasePickPhotoActivity implements
    ...
    protected void onCreate(Bundle arg0) {
        super.onCreate(arg0);
        isUpdateBankInfo = getIntent().getBooleanExtra(INTENT_UPDATE_BANK_INFO,
                false);
        initView();
...
//                }
    ...
    public void onClick(View v) {
        ...
        case R.id.act_fill_card_info_bank_name_llt: // 银行名称选择
            // TODO
            PopViewHelper.showSelectBankNamePopWindow(this, lltBankName,
                    bankNames, onBankNameSelectedListener);
            break;
        case R.id.act_fill_card_info_open_account_city_llt: // 开户行城市选择
            // TODO
            PopViewHelper.showSelectBankProvinceOrCityPopWindow(this,
                    lltOpenAccountBankCity, provinces,
                    onProvinceSelectedListener);
            break;
        case R.id.act_fill_card_info_banK_img:
            PopViewHelper.showUpdateAvatarPopupWindow(this, getWindow()
                    .getDecorView(), this);
            break;
        }
    }
    ...
    public void getBankInfo() {
        ...
        actionImpl.getBindBankInfo(userId, new ResultHandlerCallback() {
            ...
            public void rc0(RequestEntity entity, Result result) {
                BindBankCardInfoBean bankInfo = JSON.parseObject(
                        result.getData(), BindBankCardInfoBean.class);
                rendUI(bankInfo);
            }
        });
    }
    ...
    private void rendUI(BindBankCardInfoBean bankInfo) {
        if (bankInfo != null) {
            ...
            tvAddr.setText(bankInfo.getBankCity());
            String url = bankInfo.getShowImage();
            if (!TextUtils.isEmpty(url)) {
                imgBank.setScaleType(ScaleType.CENTER_CROP);
                ImageAction.displayImage(url, imgBank);
            }
            imgBank.setTag(bankInfo.getBankImage());
        }
    }

    private void commitBankInfo(String realName, String bankCode, String bankName,
            String openAccountBankName, String cityName, String bankImageUrl) {
        String userId = ContentUtils.getUserId(this);

        if ((bankFile == null || TextUtils.isEmpty(bankImageUrl))
                && !isUpdateBankInfo) {
        ...
        actionImpl.commitFeeHunterBankInfo(userId, realName, bankCode,
                ...
                new ResultHandlerCallback() {
                    ...
                    public void rc0(RequestEntity entity, Result result) {
                         ...
                         */
                        ContentUtils.updateUserType(
                                FillBankCardInfoActivity.this, 2);
                        Jumper.newJumper()
                                .setAheadInAnimation(
    ...
    private void getProvinceOrCity(final String cityCode) {
        ...
        actionImpl.getBankProvinceOrCity(cityCode, new ResultHandlerCallback() {
            ...
            @Override
            public void rc0(RequestEntity entity, Result result) {
                ArrayList<ProvinceCityBean> temp = (ArrayList<ProvinceCityBean>) JSON
                        .parseArray(result.getData(), ProvinceCityBean.class);
                if (TextUtils.isEmpty(cityCode)) {
                    provinces.addAll(temp);
                } else {
                    cities.clear();
                    cities.addAll(temp);
                    PopViewHelper.showSelectBankProvinceOrCityPopWindow(
                            FillBankCardInfoActivity.this,
                            lltOpenAccountBankCity, cities,
    ...
    private void getBankNameList() {
        ...
        actionImpl.getBankNameList(new ResultHandlerCallback() {
            ...
            @Override
            public void rc0(RequestEntity entity, Result result) {
                ArrayList<String> temp = (ArrayList<String>) JSON.parseArray(
    ...
    private void upLoadBankImage(File file) {
        ...
        final String bankName = tvBankName.getText().toString();
        if (TextUtils.isEmpty(bankName) && !isUpdateBankInfo) {
            ToastUtils.SHORT.toast(this, "请选择银行名称");
            return;
        }

        final String openAccountBankName = edtOpenAccountBankName.getText()
                .toString();
        ...
        final String cityName = tvAddr.getText().toString();
        if (!isUpdateBankInfo && TextUtils.isEmpty(cityName)) {
            ToastUtils.SHORT.toast(this, "请选择开户行城市");
        }
//        if (isUpdateBankInfo) {
//            if (city != null) {
//                cityNameTemp = city.getName();
//            }
//        } else {
//            if (city == null) {
//                ToastUtils.SHORT.toast(this, "请选择开户行城市");
...
//        }
        ...
        if (file == null && isUpdateBankInfo) {
            String url = (String) imgBank.getTag();
            commitBankInfo(realName, bankCode, bankName, openAccountBankName, cityName, url);
        } else {
            ActionImpl actionImpl = ActionImpl.newInstance(this);
            actionImpl.otherFileUpload(file, new ResultHandlerCallback() {
                ...
                public void rc0(RequestEntity entity, Result result) {
                    // TODO Auto-generated method stub
                    FileUploadResultBean fileUploadResultBean = JSON
                            .parseObject(result.getData(),
                                    FileUploadResultBean.class);
                    commitBankInfo(realName, bankCode, bankName, openAccountBankName, cityName, fileUploadResultBean.getFilePath());
                }
            });
        }
    }
