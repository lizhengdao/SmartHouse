...
import cn.com.zzwfang.adapter.MyAlreadyBoughtHouseAdapter;
import cn.com.zzwfang.adapter.SeeHouseIAmCustomerAdapter;
...
//	private LinearLayout lltMyConcernHouseSourceContianer;
	...
	private TextView tvAlreadyBoughtHouse, tvSeeHouse, tvMyAttention;
	private View lineBoughtHouse, lineSeeHouse, lineMyAttention;
	
	private ListView listIAmCustomer;
	private MyAlreadyBoughtHouseAdapter myAlreadyBoughtHouseAdapter;
	private SeeHouseIAmCustomerAdapter seeHouseAdapter;
	...
	private void initView() {
		...
		listIAmCustomer = (ListView) findViewById(R.id.lst_i_am_customer);
		
		
		tvAlreadyBoughtHouse = (TextView) findViewById(R.id.tv_already_bought_house);
		tvSeeHouse = (TextView) findViewById(R.id.tv_see_house);
		tvMyAttention = (TextView) findViewById(R.id.tv_my_attention);
...
//		lltMyConcernHouseSourceContianer = (LinearLayout) findViewById(R.id.act_i_am_customer_concern_container);
		...
		lineBoughtHouse = findViewById(R.id.line_already_bought_house);
		lineSeeHouse = findViewById(R.id.line_see_house);
		lineMyAttention = findViewById(R.id.line_my_attention);
	
		myAlreadyBoughtHouseAdapter = new MyAlreadyBoughtHouseAdapter(this);
		seeHouseAdapter = new SeeHouseIAmCustomerAdapter(this);
		attentionAdapter = new AttentionAdapter(this, null);
		
		listIAmCustomer.setAdapter(myAlreadyBoughtHouseAdapter);
		
		tvBack.setOnClickListener(this);
		tvAlreadyBoughtHouse.setOnClickListener(this);
		tvSeeHouse.setOnClickListener(this);
		tvMyAttention.setOnClickListener(this);
	}
	...
	public void onClick(View v) {
		...
		case R.id.act_i_am_customer_back:
			finish();
			break;
		case R.id.tv_already_bought_house:
			lineBoughtHouse.setVisibility(View.VISIBLE);
			lineSeeHouse.setVisibility(View.INVISIBLE); 
			lineMyAttention.setVisibility(View.INVISIBLE);
			listIAmCustomer.setAdapter(myAlreadyBoughtHouseAdapter);
			break;
		case R.id.tv_see_house:
			lineBoughtHouse.setVisibility(View.INVISIBLE);
			lineSeeHouse.setVisibility(View.VISIBLE); 
			lineMyAttention.setVisibility(View.INVISIBLE);
			listIAmCustomer.setAdapter(seeHouseAdapter);
			break;
		case R.id.tv_my_attention:
			lineBoughtHouse.setVisibility(View.INVISIBLE);
			lineSeeHouse.setVisibility(View.INVISIBLE); 
			lineMyAttention.setVisibility(View.VISIBLE);
			listIAmCustomer.setAdapter(attentionAdapter);
			break;
		}
	}
	...
	private void refreshMyBuyHouseUI() {
	    if (myBoughthouses != null && myBoughthouses.size() > 0) {
	    	myAlreadyBoughtHouseAdapter.refreshData(myBoughthouses);
...
//	        }
    ...
    private void refreshSeeHouseExperienceUI() {
        if (seeHouseExperiences != null && seeHouseExperiences.size() > 0) {
        	...
        	seeHouseAdapter.refreshData(seeHouseExperiences);
...
//	    lltMyConcernHouseSourceContianer.removeAllViews();
	    if (attentions != null && attentions.size() > 0) {
	    	...
	    	attentionAdapter.refreshData(attentions);
